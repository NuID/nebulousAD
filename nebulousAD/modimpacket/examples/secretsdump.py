...
from nebulousAD.modimpacket import winregistry, ntlm
from nebulousAD.modimpacket.dcerpc.v5 import transport, rrp, scmr, wkst, samr, epm, drsuapi
from nebulousAD.modimpacket.dcerpc.v5.dtypes import NULL
from nebulousAD.modimpacket.dcerpc.v5.rpcrt import RPC_C_AUTHN_LEVEL_PKT_PRIVACY, DCERPCException, RPC_C_AUTHN_GSS_NEGOTIATE
from nebulousAD.modimpacket.dcerpc.v5.dcom import wmi
from nebulousAD.modimpacket.dcerpc.v5.dcom.oaut import IID_IDispatch, string_to_bin, IDispatch, DISPPARAMS, DISPATCH_PROPERTYGET, \
    VARIANT, VARENUM, DISPATCH_METHOD
from nebulousAD.modimpacket.dcerpc.v5.dcomrt import DCOMConnection, OBJREF, FLAGS_OBJREF_CUSTOM, OBJREF_CUSTOM, OBJREF_HANDLER, \
    OBJREF_EXTENDED, OBJREF_STANDARD, FLAGS_OBJREF_HANDLER, FLAGS_OBJREF_STANDARD, FLAGS_OBJREF_EXTENDED, \
    IRemUnknown2, INTERFACE
from nebulousAD.modimpacket.ese import ESENT_DB
from nebulousAD.modimpacket.dpapi import DPAPI_SYSTEM
from nebulousAD.modimpacket.smb3structs import FILE_READ_DATA, FILE_SHARE_READ
from nebulousAD.modimpacket.nt_errors import STATUS_MORE_ENTRIES
from nebulousAD.modimpacket.structure import Structure
from nebulousAD.modimpacket.winregistry import hexdump
from nebulousAD.modimpacket.uuid import string_to_bin
from nebulousAD.modimpacket.crypto import transformKey
from nebulousAD.modimpacket.krb5 import constants
from nebulousAD.modimpacket.krb5.crypto import string_to_key
try:
    ...
    def __decryptHash(self, record, prefixTable=None, outputFile=None):
        ...
        if self.__useVSSMethod is True:
            ...
            if record[self.NAME_TO_INTERNAL['pwdLastSet']] is not None:
                ...
                pwdLastSet = 'N/A'
            # When we create the object, create all the necessary keys for later.
            self.secrets_dict[userName.decode('utf-8')] = {}
            self.secrets_dict[userName.decode('utf-8')]["Check"] = {
                "Compromised": None,
                "LastCheck": None,
                "Which": []
            }
            self.secrets_dict[userName.decode('utf-8')]["NTLM_Hash"] = hexlify(NTHash)
            answer = "%s:%s:%s:%s:::" % (userName, rid, hexlify(LMHash), hexlify(NTHash))
            if self.__pwdLastSet is True:
                self.secrets_dict[userName.decode('utf-8')]["PwdLastSet"] = pwdLastSet
                answer = "%s (pwdLastSet=%s)" % (answer, pwdLastSet)
            if self.__printUserStatus is True:
                self.secrets_dict[userName.decode('utf-8')]["Status"] = userAccountStatus
        ...
        try:
            ...
            if self.__useVSSMethod:
                ...
                if self.__PEK is not None:
                    ...
                    i = 0
                    maxLen = len(self.__ESEDB.getNextRow(self.__cursor))
...
def _print_helper(*args, **kwargs):
    if LOG.level == "DEBUG":
        print args[-1]  # Only print off hashes into STDOUT if DEBUG flag is set.
